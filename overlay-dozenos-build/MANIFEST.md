...
| `suffix-openssl-version.sh` | Append `+dozenos1` to the openssl changelog version inside `build_cmd` so our FIPS-patched rebuild is strictly newer than Debian's identically-versioned binary. Without it, once the Debian archive serves the recipe's exact version (bookworm point release 2026-07-11 moved `3.0.20-1~deb12u2` into main, the suite debootstrap installs from), apt sees two same-version instances with different hashes, prefers the 999-pinned repo, classifies the switch as a DOWNGRADE and every flavor's `lb build` dies -- and at equal versions apt may silently keep Debian's non-FIPS binary. openssl is the only debootstrap-base-set package built from a same-version Debian source (audited: the other salsa-pinned recipes pin versions bookworm doesn't ship, or install fresh from the 999 repo, where no downgrade classification exists). | outage 2026-07-11/12 |
